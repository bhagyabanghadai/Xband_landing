...
function FeatureCardDesktop({ feature, index, progress, total }: { feature: typeof features[0], index: number, progress: MotionValue<number>, total: number }) {
  ...
  return (
    ...
    >
      <div className="premium-card w-full max-w-lg aspect-square p-8 lg:p-12 flex flex-col justify-between bg-white border border-slate-200 rounded-[2rem] shadow-2xl">
        <div>
        ...
        <div className="w-full h-40 rounded-2xl overflow-hidden relative bg-slate-50 border border-slate-100 mt-8">
           ...
           <div className="absolute inset-0 flex items-center justify-center">
             <div className="w-20 h-20 rounded-full bg-white shadow-lg flex items-center justify-center p-2">
               <div className={`w-full h-full rounded-full bg-gradient-to-r ${feature.gradient} shadow-inner`} />
             </div>
           </div>
        </div>
      </div>
...
export default function StickyFeatures() {
  ...
  if (isMobile) {
    return (
      <section ref={containerRef} className="py-24 bg-[#FAFAFA] px-6">
        <div className="mb-16">
          <span className="text-mono-telemetry block mb-4">Platform Capabilities</span>
          <h2 className="text-4xl font-bold tracking-tight mb-6">
            The standard for <br/>
            <span className="text-gradient">global setup.</span>
          </h2>
        ...
        <div className="flex flex-col gap-8">
          {features.map((feature) => (
             <div key={feature.id} className="premium-card p-8 flex flex-col justify-between bg-white border border-slate-200 rounded-3xl shadow-lg">
               <div>
               ...
               <div className="w-full h-32 rounded-xl overflow-hidden relative bg-slate-50 border border-slate-100">
                  <div className={`absolute inset-0 opacity-20 bg-gradient-to-r ${feature.gradient}`} />
               </div>
             </div>
          ))}
  ...
  return (
    <section ref={containerRef} className="relative bg-[#FAFAFA]" style={{ height: "400vh" }}>
      <div className="sticky top-0 h-screen w-full flex items-center overflow-hidden">

        {/* Left Side: Sticky Content */}
        <div className="w-1/2 h-full flex flex-col justify-center px-12 lg:px-24">
          <div className="inline-block mb-6">
            <span className="text-mono-telemetry px-4 py-2 bg-slate-100 rounded-full font-bold">Platform Capabilities</span>
          </div>
        ...
        <div className="w-1/2 h-full relative bg-slate-100/50 border-l border-slate-200/50">
